...
[Rsync Blog](https://www.joinc.co.kr/w/Site/Tip/Rsync)
...
`rsync` 는 타겟 폴더에 소스 파일들이 이미 존재하여 변경 사항을 복사할 때 큰 효율성이 보일 것으로 기대된다. 또는 네트워크 대역폭에 제한이 있을 때 압축해서 복사하는 장점이 있다.

새로운 파일을 전체 복사하는 경우에는 빠르게 진행될지는 모르겠다.